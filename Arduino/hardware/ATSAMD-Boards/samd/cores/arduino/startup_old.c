 ...
 * We need to:
 * 1) Enable X/OSC32K clock (External on-board or internal 32.768Hz oscillator), will be used as DFLL48M reference.
 * 2) Put X/OSC32K as source of Generic Clock Generator 1
...
//
// CSL Boards add the option to use either the internal or external clock. This option can be selected in the Tools->Clock drop-down menu. 
//
...
#elseif ARDUINO_EXT32KOSC
  SYSCTRL->XOSC32K.bit.CALIB =
      ((*(uint32_t *)SYSCTRL_FUSES_OSC32K_ADDR >> 
      SYSCTRL_FUSES_OSC32K_Pos) & 0x7Ful);
